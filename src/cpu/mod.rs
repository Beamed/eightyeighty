...
pub fn decode_instructions(mut opcodes: VecDeque<u8>) -> Vec<Instruction>
{
    let mut decoded_instructions = vec!();
    while opcodes.len() > 0 {
        let op = opcodes.pop_front().expect("Error parsing opcodes. Should not have been empty.");
        //giant match below to match EVERY POSSIBLE OP
        //so, uh, don't read unless you have to
        decoded_instructions.push(
            match op {
                0x00 => Instruction::NOP,
                0x01 => get_lxi(RegisterPair::BC, opcodes.pop_front(), opcodes.pop_front()),
                ...
                0x06 => Instruction::MVI(Register::B, opcodes.pop_front().expect(MVI_ERROR)),
                ...
                0x0e => Instruction::MVI(Register::C, opcodes.pop_front().expect(MVI_ERROR)),
                0x0f => Instruction::RRC,
                0x11 => get_lxi(RegisterPair::DE, opcodes.pop_front(), opcodes.pop_front()),
                ...
                0x16 => Instruction::MVI(Register::D, opcodes.pop_front().expect(MVI_ERROR)),
                ...
                0x1e => Instruction::MVI(Register::E, opcodes.pop_front().expect(MVI_ERROR)),
                0x1f => Instruction::RAR,
                0x20 => Instruction::RIM,
                0x21 => get_lxi(RegisterPair::HL, opcodes.pop_front(), opcodes.pop_front()),
                0x22 => Instruction::SHLD(get_addr(&mut opcodes)),
                0x23 => Instruction::INX(RegisterPair::HL),
                0x24 => Instruction::INR(Register::H),
                0x25 => Instruction::DCR(Register::H),
                0x26 => Instruction::MVI(Register::H, opcodes.pop_front().expect(MVI_ERROR)),
                0x27 => Instruction::DAA,
                0x29 => Instruction::DAD(RegisterPair::HL),
                0x2a => Instruction::LHLD(get_addr(&mut opcodes)),
                0x2b => Instruction::DCX(RegisterPair::HL),
                0x2c => Instruction::INR(Register::L),
                0x2d => Instruction::DCR(Register::L),
                0x2e => Instruction::MVI(Register::L, opcodes.pop_front().expect(MVI_ERROR)),
                0x2f => Instruction::CMA,
                0x30 => Instruction::SIM,
                0x31 => get_lxi(RegisterPair::SP, opcodes.pop_front(), opcodes.pop_front()),
                0x32 => Instruction::STA(get_addr(&mut opcodes)),
                0x33 => Instruction::INX(RegisterPair::SP),
                0x34 => Instruction::INR(Register::M),
                0x35 => Instruction::DCR(Register::M),
                0x36 => Instruction::MVI(Register::M, opcodes.pop_front().expect(MVI_ERROR)),
                0x37 => Instruction::STC,
                0x39 => Instruction::DAD(RegisterPair::SP),
                0x3a => Instruction::LDA(get_addr(&mut opcodes)),
                0x3b => Instruction::DCX(RegisterPair::SP),
                0x3c => Instruction::INR(Register::A),
                0x3d => Instruction::DCR(Register::A),
                0x3e => Instruction::MVI(Register::A, opcodes.pop_front().expect(MVI_ERROR)),
                ...
                0xc2 => Instruction::JCOND(Condition::NZ, get_addr(&mut opcodes)),
                0xc3 => Instruction::JMP(get_addr(&mut opcodes)),
                0xc4 => Instruction::CCOND(Condition::NZ, get_addr(&mut opcodes)),
                0xc5 => Instruction::PUSH(RegisterPair::BC),
                0xc6 => Instruction::ADI(opcodes.pop_front().expect("ADI Opcode expected 2nd byte")),
                0xc7 => Instruction::RST(0x00),
                0xc8 => Instruction::RETCOND(Condition::Z),
                0xc9 => Instruction::RET,
                0xca => Instruction::JCOND(Condition::Z, get_addr(&mut opcodes)),
                0xcc => Instruction::CCOND(Condition::Z, get_addr(&mut opcodes)),
                0xcd => Instruction::CALL(get_addr(&mut opcodes)),
                0xce => Instruction::ACI(opcodes.pop_front().expect("ACI Opcode expected 2nd byte")),
                0xcf => Instruction::RST(0x01),
                0xd0 => Instruction::RETCOND(Condition::NC),
                0xd1 => Instruction::POP(RegisterPair::DE),
                0xd2 => Instruction::JCOND(Condition::NC, get_addr(&mut opcodes)),
                0xd3 => Instruction::OUT(opcodes.pop_front().expect("Expect byte to write to OUT")),
                0xd4 => Instruction::CCOND(Condition::NC, get_addr(&mut opcodes)),
                0xd5 => Instruction::PUSH(RegisterPair::DE),
                0xd6 => Instruction::SUI(opcodes.pop_front().expect("Expect byte to SUI")),
                0xd7 => Instruction::RST(0x02),
                0xd8 => Instruction::RETCOND(Condition::C),
                0xda => Instruction::JCOND(Condition::C, get_addr(&mut opcodes)),
                0xdb => Instruction::IN(opcodes.pop_front().expect("Expect byte (port) to read from")),
                0xdc => Instruction::CCOND(Condition::C, get_addr(&mut opcodes)),
                0xde => Instruction::SBI(opcodes.pop_front().expect("Expect byte for SBI")),
                0xdf => Instruction::RST(0x03),
                0xe0 => Instruction::RETCOND(Condition::PO),
                0xe1 => Instruction::POP(RegisterPair::HL),
                0xe2 => Instruction::JCOND(Condition::PO, get_addr(&mut opcodes)),
                0xe3 => Instruction::XTHL,
                0xe4 => Instruction::CCOND(Condition::PO, get_addr(&mut opcodes)),
                0xe5 => Instruction::PUSH(RegisterPair::HL),
                0xe6 => Instruction::ANI(opcodes.pop_front().expect("Expect byte for ANI")),
                0xe7 => Instruction::RST(0x04),
                0xe8 => Instruction::RETCOND(Condition::PE),
                0xe9 => Instruction::PCHL,
                0xea => Instruction::JCOND(Condition::PE, get_addr(&mut opcodes)),
                0xeb => Instruction::XCHG,
                0xec => Instruction::CCOND(Condition::PE, get_addr(&mut opcodes)),
                0xee => Instruction::XRI(opcodes.pop_front().expect("Expect byte for XRI")),
                0xef => Instruction::RST(0x05),
                0xf0 => Instruction::RETCOND(Condition::P),
                0xf1 => Instruction::POP_PSW,
                0xf2 => Instruction::JCOND(Condition::P, get_addr(&mut opcodes)),
                0xf3 => Instruction::DI,
                0xf4 => Instruction::CCOND(Condition::P, get_addr(&mut opcodes)),
                0xf5 => Instruction::PUSH_PSW,
                0xf6 => Instruction::ORI(opcodes.pop_front().expect("Expect byte for ORI")),
                0xf7 => Instruction::RST(0x06),
                0xf8 => Instruction::RETCOND(Condition::M),
                0xf9 => Instruction::SPHL,
                0xfa => Instruction::JCOND(Condition::M, get_addr(&mut opcodes)),
                0xfb => Instruction::EI,
                0xfc => Instruction::CCOND(Condition::M, get_addr(&mut opcodes)),
                0xfe => Instruction::CPI(opcodes.pop_front().expect("Expected argument for CPI")),
                0xff => Instruction::RST(0x07),
                _ => panic!("Unable to parse op: {}", op)
            }
        )
    }
